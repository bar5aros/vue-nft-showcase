<template>
  <div class="card" v-b-hover="handleCardHover" :style="isHovered ? hoveredStyle : {} ">
    <b-card no-body
    ...
    >
      ...
      <b-card-body>
        <b-card-text style="font-family: 'Outfit', Arial, sans-serif">{{ nftName }}</b-card-text>
      ...
      <b-card-footer style="text-align: end; background: #fff;">
        <button v-on:click="handleFavorite">
          <font-awesome-icon icon="fa-solid fa-heart" v-if="isFavorite" class="favorite-icon"/>
          <font-awesome-icon icon="fa-regular fa-heart" v-else class="favorite-icon"/>
        </button>
      </b-card-footer>
    </b-card>
  </div>
